...
function DrawHandler({
  ...
  const handleShapeDrawn = (layer) => {
    shapesArrayRef.current.push(layer); // store reference to drawn shape
    ...
    if (!controlAdded) {
      ...
      const drawControl = new L.Control.Draw({
        draw: {
          ...
          circlemarker: false, // circlemarker disabled
      ...
      setDeleteFunction(() => {
        if (shapesArrayRef.current.length > 0) {
